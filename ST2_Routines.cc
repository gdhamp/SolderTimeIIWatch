...
shortloop:
    ...
    if(Serial.available()>0)
    {
      ...
      if(IncomingMessIndex<IN_MESSAGE_SIZE)
      {
...
void lamptest()
{
  ...
  void GETFROMEEPROM() 
  {
    ...
    for(int EEPadd=0; EEPadd < IncomingMessIndex; EEPadd++)
    {
      IncomingMessage[EEPadd] = EEPROM.read(EEPadd+1); // the first byte of the message in EEPROM is at location 0
    }
  }
...
void FILLEEPROM()                                                      // Normally only run once if EEPROM is clear
{
  ...
  for(int EEPadd=0; EEPadd < IncomingMessIndex; EEPadd++)
  {
    ...
    EEPROM.write(EEPadd+1, IncomingMessage[EEPadd]);
  }
